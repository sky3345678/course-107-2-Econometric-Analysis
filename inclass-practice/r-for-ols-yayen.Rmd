...
#要習慣用一個自己的Series，pipe不錯


tempDF %>%
  pairs.panels()

# ?dplyr::select  tbl is dataframe
```
...
#注意NA，以免之後filter might be complex
# 進階關連分析
...
```{r}
#第二堂課蠻重要10.50之後
#V(beta hate) = sigma^2(x'x)^-1  有可能會考
...
modelCoe1 <- tidy(model_1_coeftest)

coeftest(model_2, vcov. = vcovHC, type="HC1") -> model_2_coeftest

modelCoe2 <- tidy(model_2_coeftest)
